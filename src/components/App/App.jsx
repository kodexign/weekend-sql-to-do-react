...
//functioning but the list gets re-ordered each time it toggles
  ...
  return (
    <div className="App">
      ...
      <h2>Task List</h2>
      {taskArray.map((todo) => { return (<li key={todo.task}>{todo.task} {todo.complete} <button onClick={() => deleteTask(todo.id)}>Remove</button> <button onClick={() => toggleTask(todo.id)}>{todo.complete ? "completed" : "not complete"}</button></li>); })}
    </div>
  );
}
